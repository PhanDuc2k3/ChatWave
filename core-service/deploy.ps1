...
param(
    ...
    [string]$User,
    [Parameter(Mandatory=$false)]
    [string]$RemotePath = "/opt/core-service"
)

$LocalPath = $PSScriptRoot

# Load .env từ core-service hoặc chatbot-service
...
Write-Host "===> Deploy core-service to $User@${ServerHost}:$RemotePath" -ForegroundColor Cyan
...
Run-Scp -r -o StrictHostKeyChecking=no "$LocalPath\proto" $dest/
...
Write-Host ""
Write-Host "===> Deploy xong! Core API: http://${ServerHost}:5001/api/v1" -ForegroundColor Green
Write-Host "Cập nhật frontend: VITE_API_BASE_URL=http://${ServerHost}:5001/api/v1" -ForegroundColor Cyan
